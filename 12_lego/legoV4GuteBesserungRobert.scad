...
module impText() {
    translate([20, 30, 0])
    import("emText_Gute.stl");
    translate([0, 15, 0])
    import("emText_Besserung.stl");
    translate([15, 0, 0])
    import("emText_Robert.stl");
}

module carryText(loc_res = 20, show_text = 1) {
    difference() {
        hull() {
            translate([0, -12, 0])
            sphere(d = 6, $fn = loc_res);
            translate([0, +12, 0])
            sphere(d = 6, $fn = loc_res);
            translate([0, -12, 14])
            sphere(d = 6, $fn = loc_res);
...
module legoFigure(loc_res = 20, show_origin = 1, show_face = 1, complex_hand = 1, arm_angle = 0, show_text = 1) {
    if(show_origin) {
        translate([0, 0, 0])
        sphere(d = 1, $fn = loc_res);
    }
...
// normal figure
//scale([2, 2, 2])
//legoFigure(32, 0, 1, 1, 10);
//legoFigure(10, 0, 1, 0, 10, show_text = 0);
//legoFigure(10, 0, 0, 0, 10, show_text = 0);

// outline + 1mm
minkowski() {
    import("legoV4GuteBesserungRobertFigure2Low.stl");
    sphere(d = 1.3, $fn = 20);
}
